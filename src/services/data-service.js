...
const items = [
  ...
  },
  {
    ...
    price: 400,
    descr:
      "Kомпьютерная игра в жанре шутера от первого лица, разработанная компанией Bungie и изданная Microsoft Game Studios. Изначально выпущена в 2001 году для игровой приставки Xbox. Через два года вышла версия для Microsoft Windows. В том же году выпущена новелла-предыстория Halo: The Fall of Reach. ",
    discount: null,
    requirements: [
      {
        name: "ОС",
    ...
    similiarGames: ["halo_1", "halo_2", "halo_3"],
    gallery: [
  ...
  },
  {
    ...
    descr:
      "Kомпьютерная игра в жанре шутера от первого лица, созданная компанией Bungie Studios. Игра является продолжением истории, начатой в Halo: Combat Evolved и продолженной в Halo 2. Игра была выпущена в 2007 году для игровой консоли Xbox 360, в 2014 году была переиздана в составе Halo: Master Chief Collection для Xbox One, а в 2020 году выпущена на персональных компьютерах под управлением Windows в том же комплекте. ",
    price: 500,
    discount: null,
    requirements: [
      {
  ...
  },
  {
    ...
    price: 700,
    descr:
      "Компьютерная игра в жанре шутера от первого лица, разработанная компанией 343 Industries для игровой консоли Xbox 360. Это первая игра из «Reclaimer Saga» — новой истории в серии Halo. Игра выпущена во всём мире 6 ноября 2012 года, за исключением Японии, где игра была выпущена 8 ноября. Действие игры начинается через четыре года после событий Halo 3, и отмечена возвращением главного героя серии — Мастера Чифа. ",
    discount: null,
    requirements: [
      {
        name: "ОС",
  ...
  },
  {
    ...
    src:
      "https://upload.wikimedia.org/wikipedia/en/thumb/6/62/Amnesia-The-Dark-Descent-Cover-Art.png/220px-Amnesia-The-Dark-Descent-Cover-Art.png",
    price: 200,
    descr:
      "Трёхмерная компьютерная игра в жанре survival horror, разработанная шведской командой Frictional Games, ранее ставшей известной разработкой серии игр Penumbra. Выпуск игры состоялся 8 сентября 2010 года на персональных компьютерах[2]",
    discount: null,
    requirements: [
      {
        name: "ОС",
    ...
    service: "Steam",
    languageSupport: true,
    similiarGames: ["penumbra", "outlast", "outlast_2"],
    gallery: [
  ...
  },
  {
    ...
    src:
      "https://upload.wikimedia.org/wikipedia/en/thumb/2/21/Penumbra-win-cover.jpg/220px-Penumbra-win-cover.jpg",
    price: 200,
    descr:
    ...
    service: "Steam",
    languageSupport: false,
    similiarGames: ["amnesia_the_dark_descent", "outlast", "outlast_2"],
    gallery: [
  ...
  },
  {
    name: "Outlast",
    src:
      "https://upload.wikimedia.org/wikipedia/en/thumb/a/aa/Outlast_cover.jpg/220px-Outlast_cover.jpg",
    price: 500,
    descr:
    ...
    service: "Steam",
    languageSupport: true,
    similiarGames: ["penumbra", "amnesia_the_dark_descent", "outlast_2"],
    gallery: [
  ...
  },
  {
    ...
    service: "Steam",
    languageSupport: true,
    similiarGames: ["penumbra", "outlast", "amnesia_the_dark_descent"],
    gallery: [
...
export default class DataService {
  ...
  getBestsellers() {
    let res = [];
    items.forEach((ele) => {
      if (bestsellers.includes(ele.id)) {
        res.push(ele);
      }
    });
  ...
  getFilteredItems(filter) {
    ...
    items.map((item) => {
      ...
      if (calculatedPrice > maxPrice || calculatedPrice < minPrice) {
        return false;
      }

      if (
        genres.every((genre) => item.genres.includes(genre)) === false &&
        genres.length !== 0
      ) {
        return false;
      }
      if (
        developers.some((developer) => item.developer === developer) ===
          false &&
        developers.length !== 0
      ) {
        return false;
      ...
      if (
        publishers.some((publisher) => item.publisher === publisher) ===
          false &&
        publishers.length !== 0
      ) {
        return false;
      ...
      if (
        services.some((service) => item.service === service) === false &&
        services.length !== 0
      ) {
        return false;
      }
      if (
        regions.some((region) => item.region === region) === false &&
        regions.length !== 0
      ) {
        return false;
      }
  ...
  getAllPublishers() {
    let res = [];
    items.forEach((item) => {
      if (res.includes(item.publisher) === false) {
        res.push(item.publisher);
      }
    });
    return res;
  }
  ...
  getAllDevelopers() {
    let res = [];
    items.forEach((item) => {
      if (res.includes(item.developer) === false) {
        res.push(item.developer);
      }
    });
    return res;
  }
  ...
  getAllRegions() {
    let res = [];
    items.forEach((item) => {
      if (res.includes(item.region) === false) {
        res.push(item.region);
      }
    });
    return res;
  }
  ...
  getAllServices() {
    let res = [];
    items.forEach((item) => {
      if (res.includes(item.service) === false) {
        res.push(item.service);
      }
    });
    return res;
  }
  ...
  getAllGenres() {
    let res = [];
    items.forEach((item) => {
      if (res.includes(item.genre) === false) {
        res.push(item.genre);
      }
    });
    return res;
